...
import { createProductData } from "../../network/product_api";
...
const AddProduct = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      try {
        ...
        setCategories(processCategoryHierarchy(response.data));
      } catch (error) {
        console.error("Error fetching categories:", error);
        setErrors(prev => ({
          ...prev,
          categories: "Failed to load categories. Please try refreshing the page."
        }));
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({ 
        submit: error.response?.data?.message || "Failed to create product. Please try again." 
      });
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5">
      <div className="w-full">
        ...
        <form onSubmit={handleSubmit} className="bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8 mt-4">
          ...
          {/* Enhanced Category selection field */}
          <div className="mb-4">
            <label
              htmlFor="categoryId"
              className="block mb-2 text-sm font-medium text-gray-900"
